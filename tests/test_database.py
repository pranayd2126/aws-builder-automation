...
from app.config import APPROVED_COMMENTS

def test_consecutive_comment_prevention_and_exact_strings(memory_db):
    """Test consecutive comment prevention, exact string selection, and rotation."""
    # Ensure there are comments to test
    assert len(APPROVED_COMMENTS) >= 2, "Need at least 2 configured comments for rotation tests"

    # 1. Get first comment
    c1 = memory_db.get_next_comment(APPROVED_COMMENTS, is_dry_run=False)
    assert c1 in APPROVED_COMMENTS, "Selected comment is not exactly in the approved list"
    ...
    assert c2 in APPROVED_COMMENTS, "Selected comment is not exactly in the approved list"
    assert c2 != c1, "Comment was unnecessarily selected consecutively"
    ...
    if len(APPROVED_COMMENTS) >= 3:
        c3 = memory_db.get_next_comment(APPROVED_COMMENTS, is_dry_run=False)
        assert c3 in APPROVED_COMMENTS, "Selected comment is not exactly in the approved list"
        assert c3 != c2, "Comment was unnecessarily selected consecutively"
